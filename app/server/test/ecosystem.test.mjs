...
// 格式提示正是旧路径失败的诱因，锁住已经取得响应的自然语言请求。
test('解说只发一次 user 消息，原样保留文本，严格复用附录 prompt', async () => {
  let calls = 0;
  const result = await ecosystem.explainEcosystem(question, [item], options({ chat: async (model, messages) => {
    calls++;
    assert.equal(model, 'zhida-thinking-1p5');
    assert.deepEqual(messages, [{ role: 'user', content: `在知乎问题「${question}」下，目前主要存在哪几种不同立场的观点？\n每一派分别靠什么方式说服读者——是摆数据、讲个人经历、调动情绪、诉诸身份认同、引用权威，还是靠抖机灵？\n请分派说明，并指出哪一派目前最占上风。` }]);
    return narrative;
  } }));
  assert.equal(calls, 1);
  assert.equal(result.aiNarrative, narrative);
  assert.equal(result.aiNarrativeSource, 'zhida-thinking-1p5');
...
test('放生立场由草稿决定，AI 不参与规则分值/风险/建议', async () => {
  const draft = item.excerpt.repeat(30);
  const report = await ecosystem.analyzeRelease(question, [], draft, options({ chat: async (_model, messages) => {
    assert.equal(messages.length, 1);
    assert.equal(messages[0].role, 'user');
    assert.ok(messages[0].content.includes(draft.slice(0, 800)));
    assert.ok(!messages[0].content.includes(draft.slice(0, 801)));
    return '{"survivalProbability":0,"risks":["AI 覆盖"],"stance":"反对"}';
  } }));
  ...
  assert.match(report.aiComment, /survivalProbability/);
